if [ $# -lt 5 ]
then
    echo "Usage: sh getRocPeak.sh peakfile cell posFile negFile suffix"
    exit
fi
...
#cp $file $sortFile
numP=`windowBed -a $sortFile -b $posfile -u -w 0 |wc -l`
numN=`windowBed -a $sortFile -b $negfile -v -w 0 |wc -l`
for ((i=1;i<=10;i++)) do
    now=`echo $one*$i|bc`
    head -n $now $sortFile >tempRoc$suffix
    nowP=`windowBed -a tempRoc$suffix -b $posfile -u -w 0 |wc -l`
    nowN=`windowBed -a tempRoc$suffix -b $negfile -v -w 0 |wc -l`
